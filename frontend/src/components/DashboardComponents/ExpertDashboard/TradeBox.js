...
function TradeBox() {
    ...
    const { setWalletData } = useContext(DashboardContext)
                ...
                if (res.data.success) {
                    await axios.post(`/transaction/buyCoin/${res.data.data._id}`, { coin: inputButtonValue, coinCount: values.coinCount, currentCoinPrice: currentCoinPrice }, { headers }).then(async (res) => {
                        if (res.data.success) {
                            ...
                            await axios.get("/user/fetchFollowers", { headers }).then(async (res) => {
                                if (res.data.success) {
                                    await Promise.all(
                                        res.data.data.map(async (key, i) => {
                                            await axios.post(`/transaction/buyCoin/${key._id}`, { coin: inputButtonValue, coinCount: (key.followedPercentage ? values.coinCount * key.followedPercentage / 100 : values.coinCount * 0.1), currentCoinPrice: currentCoinPrice }, { headers })
                                        })
                                    )
                                } else {
                        ...
                        if (res.data.success) {
                            ...
                            await axios.get("/user/fetchFollowers", { headers }).then(async (res) => {
                                if (res.data.success) {
                                    await Promise.all(
                                        res.data.data.map(async (key, i) => {
                                            await axios.post(`/transaction/sellCoin/${key._id}`, { coin: inputButtonValue, sellCount: (key.followedPercentage ? values.coinCount * key.followedPercentage / 100 : values.coinCount * 0.1), currentCoinPrice: currentCoinPrice }, { headers })
                                        })
                                    )
                                } else {
                                    toastErrorMessage(res.data.message);
